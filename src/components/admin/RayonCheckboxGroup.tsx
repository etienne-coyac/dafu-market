...
const RayonCheckboxGroup: React.FC<Props> = ({
  rayon,
  categories,
  onRayonChange,
}) => {
  const allChecked: boolean = categories.every(
    (cat: CategoryType) => cat.checked
  );
  const someChecked: boolean = categories.some(
    (cat: CategoryType) => cat.checked
  );
      ...
      >
        {categories.map((cat: CategoryType, index: number) => (
          <Checkbox
            ...
            checked={!!cat.checked}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              handleChildChange(index, e.target.checked)
            }
          />
        ))}
      </Box>
