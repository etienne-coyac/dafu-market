    ...
    <React.Fragment>
      <FormControl size="sm">
        ...
        <Select
          ...
          onChange={(_, newValue) => {
            setStatusFilter(
              typeof newValue === "string" && newValue !== "" ? newValue : null
            );
          }}
        >
          ...
          {(Array.isArray(commandes) ? commandes : [])
            .slice()
            .sort(getComparator(order, "jourRetrait"))
            .map((row) => (
              ...
              Array.isArray(commandes)
                ? commandes.filter((row) => {
                    const searchMatch = searchFilter
                      ? String(row.idCommande)
                          .toLowerCase()
                          .includes(searchFilter.toLowerCase())
                      : true;
                    const customerMatch = customerFilter
                      ? String(row.panier.idClient).toLowerCase() ===
                        customerFilter.toLowerCase()
                      : true;
                    const statusMatch = statusFilter
                      ? row.statut.toLowerCase() === statusFilter.toLowerCase()
                      : true;
    ...
    <React.Fragment>
      ...
      >
        ...
        >
          ...
          <tbody>
            {(((filteredRows ?? commandes) as Array<any>) ?? [])
              .slice()
              .sort(getComparator(order, "jourRetrait"))
              .map((row) => (
                <tr key={`${row.idCommande}`}>
                        ...
                        (
                          {
                            ...
                            Refunded: "neutral",
                            Cancelled: "danger",
                          } as Record<string, ColorPaletteProp>
                        )[row.statut] ?? "neutral"
                      }
                    >
                      {row.statut}
                  ...
                  <td>
                    <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                      <div>
                        {(Array.isArray(commandes) ? commandes : []).map(
                          (row) => {
