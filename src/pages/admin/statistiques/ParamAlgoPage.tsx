...
import { getAllCategoriesPreview, getTousLesRayons } from '../../../api/rayons.api';
import { patchCheckedFalse, patchCheckedTrue } from '../../../api/admin/mettrePreview.api';

type HandleRayonChange = (index: number, newCategories: RayonType['categories']) => void;

const ParamAlgo: React.FC = () => {
  const { data: rayonsData } = useQuery<RayonType[]>({
    queryKey: ['rayons'],
    queryFn: getTousLesRayons,
    });

  const { data: dataChecked } = useQuery<CategoryType[]>({
  ...
  if (rayonsData && dataChecked) {
    const checkedIds = new Set(Array.isArray(dataChecked) ? dataChecked.map(cat => cat.idCategorie) : []);
  ...
  const allCategories: CategoryType[] = rayons.flatMap((rayon) => rayon.categories);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      {rayons.map((rayon, index) => (
        <RayonCheckboxGroup
          ...
          onRayonChange={(newCategories) => handleRayonChange(index, newCategories)}
        />
      ))}
      {message && (
        <Typography level="body-sm" color={message.startsWith("✅") ? "success" : "danger"} sx={{ mt: 2 }}>
            {message}
        </Typography>
        )}
      <Button
